...
## 動画アップロードが遅い件について（高速化の改善内容）

「30枚程度でも動画があると1分くらいかかる」件、切り分けて対応しました。

**改善できない部分（物理的な制約）**
ゲストのスマホの回線の上り速度（アップロード速度）そのものは、サイト側のコードでは変えられません。特にモバイル回線（4G/5G）は上りが遅いことが多く、動画のような大きなファイルはどうしても時間がかかります。案内文に「動画が多い場合は会場のWi-Fiへの接続を推奨」という一言を追加しました。

**改善した部分（実装済み）**
- 以前は、1本の動画を10MBずつのパートに分割したあと、律儀に1パートずつ順番に送信していました。これを、**同じ動画の中の複数パートを最大3つ同時並行で送信する**方式に変更しました。1本の動画のアップロード時間そのものが体感で大きく短縮されます。
- fetchからXMLHttpRequestに切り替え、**各サムネイルに送信中のリアルタイム%表示**を追加しました。実際の速度が変わらなくても「止まっているように見える」ストレスが減ります。

体感速度をさらに求める場合、次のような追加策も可能です（今回は未実装、ご要望があれば追加します）。
- 動画をブラウザ上で軽量な画質に圧縮してから送信する（画質は落ちますが送信量を大きく減らせます。ただし処理が重く、逆にスマホが重くなる場合があります）
- ゲストに「動画は1本1〜2分以内で」とお願いする案内文を追加する
