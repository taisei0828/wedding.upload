...
const SINGLE_PUT_LIMIT = 90 * 1024 * 1024;    // 90MB以下は1リクエストでそのまま送る
const CHUNK_SIZE = 10 * 1024 * 1024;          // マルチパート時の1パートサイズ（R2の規定で5MB以上必須）
const MAX_FILE_SIZE = 2 * 1024 * 1024 * 1024; // 1ファイルあたりの上限（2GB）
const MAX_RETRIES = 4;                        // 混雑時の自動リトライ回数
const FILE_CONCURRENCY = 3;                   // 同時に何件のファイルを並行してアップロードするか
const PART_CONCURRENCY = 3;                   // 1本の大きな動画の中で、何パートを同時に並行送信するか
...
// XMLHttpRequestで送信（fetchと違い、送信中の進捗（onprogress）を取得できるため）
function xhrUpload(url, { headers = {}, body, signal, onProgress }){
    ...
    xhr.onload = () => {
      if(xhr.status >= 200 && xhr.status < 300){
        try{
          ...
          if(data.status !== 'success'){ reject(new Error(data.message || 'アップロードに失敗しました')); return; }
          resolve(data);
        }catch(e){ reject(e); }
      }else{
...
// ネットワーク不調・サーバー混雑時に自動リトライする
async function xhrWithRetry(url, options){
  let lastErr;
  for(let attempt = 0; attempt <= MAX_RETRIES; attempt++){
    if(options.signal && options.signal.aborted) throw new DOMException('aborted', 'AbortError');
    try{
      return await xhrUpload(url, options);
    }catch(err){
...
// 複数パートを同時並行で処理する簡易プール（1本の動画の中でのマルチパート送信を高速化するため）
async function runPartsWithConcurrency(indices, concurrency, worker){
  let idx = 0;
  let firstError = null;

  async function runner(){
    while(idx < indices.length){
      if(firstError || cancelRequested) return;
      const i = indices[idx++];
      try{
        await worker(i);
      }catch(err){
  ...
  const runners = [];
  for(let k = 0; k < concurrency; k++) runners.push(runner());
  await Promise.all(runners);
  if(firstError) throw firstError;
}

async function uploadItem(item, guestName, onPercent){
  const file = item.file;
  const controller = new AbortController();
  item.controller = controller;
  const uploadId = makeUploadId();

  // 進捗計算用：完了済みバイト＋現在送信中の各リクエストの送信済みバイトを合算する
  let completedBytes = 0;
  const inFlight = new Map();
  function reportProgress(){
    let sum = completedBytes;
    for(const v of inFlight.values()) sum += v;
    const pct = Math.min(100, Math.round((sum / file.size) * 100));
    onPercent(pct);
  }

  // ケース1: 90MB以下は1リクエストでそのまま送信
  if(file.size <= SINGLE_PUT_LIMIT){
    inFlight.set(0, 0);
    await xhrWithRetry(WORKER_URL + '/chunk', {
      ...
      body: file,
      onProgress: (loaded) => { inFlight.set(0, loaded); reportProgress(); },
    });
    onPercent(100);
    return;
  }
  ...
  function makePartUploader(i, r2UploadIdForThisCall){
    ...
    if(r2UploadIdForThisCall) headers['X-R2-Upload-Id'] = r2UploadIdForThisCall;

    return xhrWithRetry(WORKER_URL + '/chunk', {
      signal: controller.signal,
      headers,
      body: chunkBlob,
      onProgress: (loaded) => { inFlight.set(i, loaded); reportProgress(); },
    }).then(data => {
      inFlight.delete(i);
      completedBytes += chunkBlob.size;
      reportProgress();
      return data;
    });
  }

  // 最初のパートだけ先に送り、マルチパートアップロードを作成してr2UploadId/keyを得る
  const firstData = await makePartUploader(0, null);
  const r2UploadId = firstData.r2UploadId;
  const key = firstData.key;
  const parts = [{ partNumber: firstData.partNumber, etag: firstData.etag }];

  // 残りのパートは並行して送信（これが「動画1本あたりの体感速度」を大きく左右する）
  if(totalChunks > 1){
    const remaining = [];
    for(let i = 1; i < totalChunks; i++) remaining.push(i);

    await runPartsWithConcurrency(remaining, PART_CONCURRENCY, async (i) => {
      const data = await makePartUploader(i, r2UploadId);
  ...
  await xhrWithRetry(WORKER_URL + '/complete', {
    signal: controller.signal,
  ...
  return new Promise(resolve => {
    ...
    function tryStart(){
      ...
      while(active < concurrency && idx < items.length){
        ...
        setThumbState(item.id, 'uploading', '0%');
        uploadItem(item, guestName, (pct) => {
          if(item.status === 'uploading') setThumbState(item.id, 'uploading', pct + '%');
        })
          .then(() => {
            item.status = 'done';
...
form.addEventListener('submit', async (e) => {
  ...
  const { doneCount, failed } = await runQueueWithConcurrency(targets, guestName, FILE_CONCURRENCY, (done, total) => {
